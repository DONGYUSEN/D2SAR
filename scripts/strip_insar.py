...
def _run_nisar_dense_offsets_impl(
    *,
    ...
    if use_gpu:
        from scripts.gpu_utils import init_cuda_device
        _gpu_info = init_cuda_device(gpu_id, gpu_mode="auto")
...
def _crossmul_isce3_gpu_subprocess(
    *,
    ...
    _gpu_guard = (
        ...
        "except ImportError:\n"
        "    import sys\n"
        "    print('ERROR: isce3.cuda not available for GPU crossmul', "
        "file=sys.stderr)\n"
        "    sys.exit(49)\n"
        "\n"
    )
...
def _crossmul_filter_isce3_gpu_subprocess(
    *,
    ...
    _gpu_guard2 = (
        ...
        "except ImportError:\n"
        "    import sys\n"
        "    print('ERROR: isce3.cuda not available for GPU crossmul+filter', "
        "file=sys.stderr)\n"
        "    sys.exit(49)\n"
        "\n"
    )
...
def process_strip_insar(
    ...
    *,
    ...
    master_bursts: list[Any] | None = None,
    slave_bursts: list[Any] | None = None,
    overlaps: list[Any] | None = None,
    ...
    # Lazy import for TOPS mode — only imported when tops_mode=True
    if tops_mode:
        from strip_insar_tops import (  # type: ignore[import-unused]
            run_burst_ifg_stage,
            run_burst_merge_stage,
            run_esd_estimation_stage,
            run_tops_burst_fine_offsets_stage,
            run_tops_burst_geometry_stage,
        )
